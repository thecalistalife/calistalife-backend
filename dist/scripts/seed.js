...
const seedDatabase = async () => {
    try {
        ...
        await models_1.Collection.deleteMany({});
        await models_1.User.deleteMany({ role: 'admin' });
        console.log('🗑️  Cleared existing data');
        const collections = await models_1.Collection.insertMany(collectionsData);
        console.log(`✅ Seeded ${collections.length} collections`);
        const products = await models_1.Product.insertMany(productsData);
        console.log(`✅ Seeded ${products.length} products`);
        const admin = await models_1.User.create(adminUser);
